...
\begin{abstract}
While prior work shows post-hoc fusion of independently trained models is feasible, this paper characterises when it succeeds and by how much. Independently trained domain specialists can be fused post-hoc into a single model that outperforms any individual specialist, and the gain is predictable before training:
...
Notably, conversion efficiency---the ratio of fusion gain to specialist divergence---increases with model scale ($0.75\times$ at 6.9B vs.\ $0.49\times$ at smaller scales), suggesting the cooperative mechanism strengthens rather than degrades as models become more capable.
The fused model achieves oracle-optimal routing: the learned router matches the domain oracle with gap $<10^{-5}$ nats.
Phase 2 extends the protocol to high-divergence settings: cross-lingual fusion (Tamil/Yoruba/Welsh/Code) achieves $+21.87\%$ ($\pm$0.12pp, 3-seed mean), with Yoruba perplexity falling $41.9\to7.7$.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
  ...
  \item \textbf{High-divergence validation.} Phase 2 extends the protocol to private professional domains ($+10.17\%$, medical/legal/patent) and cross-lingual settings ($+21.87\%$ $\pm$0.12pp, 3-seed mean; Tamil/Yoruba/Welsh/Code; Yoruba perplexity $41.9\to7.7$). Gain scales most steeply where the base model is least competent.
...
\paragraph{Inference cost.} The $N\times$ inference cost is the price of aggregation rather than overhead: in settings where contributors cannot share data, the relevant comparison is not against a single centrally-trained model but against the absence of any unified cross-domain model. The \kalavai fused model runs all $N$ specialists in parallel at inference, increasing compute by a factor of $N$ for the unfrozen layers. For $N=3$ with 17\% frozen layers, the effective inference overhead is approximately $2.5\times$ (frozen layers run once; unfrozen layers run $3\times$). Measured benchmarks on an NVIDIA RTX 5090 confirm this: dense MoE latency is $2.86\times$ base at 410M and $3.35\times$ base at 1B (Table~\ref{tab:inferencebench} in Appendix~\ref{app:inference}).
